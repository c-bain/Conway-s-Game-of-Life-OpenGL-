...
int frame_counter = 0;
int frames_per_update = 3; // Update every N frames
...
void rules(){
		// Frame-based delay instead of usleep
		frame_counter++;
		int frame_skip = 1;
		if(speedup==1){frame_skip=1;} //speed control(normal)
		else if(speedup==2){frame_skip=3;}//slower
		else if(speedup==3){frame_skip=6;}//"" 
		else if(speedup==4){frame_skip=10;}//slower
